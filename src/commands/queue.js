...
/**
 * @type {import("../types/index").ICommandBuilder<import("discord.js").CacheType>}
 */
module.exports = {
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          currentTracks
            .map(
              (track, index) =>
                `${start + index + 1}. **${track.title}** - Requested by: **${
        ...
        .setFooter({
          text: `There are ${queue.tracks.length} song(s) in total in the queue.`,
        });

      if (queue.current) {
        embed.addFields({
          name: "Now Playing",
          value: `**${queue.current.title}** - Requested by: **${queue.current.requestedBy.username}**`,
        });
      }
